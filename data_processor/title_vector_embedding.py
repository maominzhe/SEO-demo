...
def create_table_if_not_exists(conn, cur):
    cur.execute("""
        CREATE TABLE IF NOT EXISTS title_vector_embeddings (
...
def encode_title_and_save_vector(conn,cur):
    ...
    for record in cur.fetchall():
        blog_post_id, title = record
        # 对标题进行编码
        vector = encoder.encode(title).tolist()

        # 将向量写入vector_embeddings表
        cur.execute(
            "INSERT INTO title_vector_embeddings (blog_post_id, vector) VALUES (%s, %s)",
            (blog_post_id, vector)
        )
...
if __name__ == "__main__":
    ...
    encode_title_and_save_vector(conn,cur)
    cur.close()
    conn.close()